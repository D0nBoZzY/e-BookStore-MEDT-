<?php
    ...
    if(isset($_POST['ID']) AND $_POST['ID'] != 0) {
        if(isset($_POST['submit']) AND $_POST['submit'] == 'User sperren') {
            // User sperren
            $sql = "UPDATE User
                SET block='blocked' where 
                ID = '".$_POST['ID']."'
                   ";
            mysql_query($sql) OR die("<pre>\n".$sql."</pre>\n".mysql_error());

            echo "Der User wurde gesperrt.<br>\n";
        }
        elseif(isset($_POST['submit']) AND $_POST['submit'] == 'User auswählen') {
            echo "Wollen Sie diesen User wirklich sperren?<br>\n";
            $sql = "SELECT
                            SessionID,
                            Nickname,
                            Email,
            ...
            echo "<form ".
                 "action=\"index.php?section=admin&page=user&action=block\" ".
                 "method=\"post\">\n";
            echo "<input type=\"hidden\" name=\"ID\" value=\"".$_POST['ID']."\">\n";
            echo "<input type=\"submit\" name=\"submit\" value=\"User sperren\">\n";
            echo "</form>\n";
        }
    }
    else {
        ...
        if(!mysql_num_rows($result))
            ...
            echo "<form ".
                 " action=\"index.php?page=user&action=block\" ".
                 " method=\"post\" ".
